...
merged11 = "C:\\FUME\\popnetv2\\data\\2011\\merged"
path17 = "C:\\FUME\\popnetv2\\data\\2017"
merged17 = "C:\\FUME\\popnetv2\\data\\2017\\merged"

#18subregion names and others
country_dict11 = {"australia" : ["aus","cxr", "cck", "hmd", "nzl", "nfk"],
            "antarctica" : ["ata"],
...
for key in country_dict11:
    for value in country_dict11[key]:
        for file in os.listdir(path11):
            #print(value)
            if file.endswith('{}.tif'.format(value)) and file.startswith('cph_2011'):
                print(key, '--',file)
                with rasterio.open('{}/{}'.format(path11, file)) as src:
                    pop_2011 = src.read(1)
                    p1 = src.crs
                    height = src.shape[0],
                    width= src.shape[1],
                    bb= src.transform,
                    pop_2011[pop_2011<0]=0
                    pop_list_2011.append(pop_2011)

    total_pop_2011=np.add.reduce(pop_list_2011)
    print(key, total_pop_2011)

    with rasterio.open('{}/cph_2011_pop_origin_new_dnk.tif'.format(path11)) as dd:

        new_dataset = rasterio.open(
             '{0}/totalPop_{1}_2011.tif'.format(merged11,key),
             'w',
             driver='GTiff',
             height=total_pop_2011.shape[0],
             width=total_pop_2011.shape[1],
             count=1,
             dtype=total_pop_2011.dtype,
             crs=p1,
             transform= dd.transform
         )
    new_dataset.write(total_pop_2011, 1)
    new_dataset.close()

#______________________
print("Creating sum files for year ....")

def sumUpTif(pop_list, dictionary, tif_path, out_folder, year):
    pop_list=[]
    ...
    for key in dictionary:
        ...
        with rasterio.open('{0}/cph_{1}_pop_origin_new_dnk.tif'.format(tif_path,year)) as dd:
            ...
            new_dataset = rasterio.open(
                 ...
                 'w',
                 driver='GTiff',
                 height=total_pop_2017.shape[0],
                 width=total_pop_2017.shape[1],
                 count=1,
                 dtype=total_pop_2017.dtype,
                 crs=p1,
                 transform= dd.transform
             )
